...
public class AdministradorController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN','ADMINISTRADOR','ENTRENADOR')")
    ...
    public ResponseEntity<Usuario> actualizar(@PathVariable Long id, @Valid @RequestBody ActualizarUsuarioRequest r){
        return ResponseEntity.ok(s.actualizarUsuario(id,r)); }
    @DeleteMapping("/usuarios/{id}")
    public ResponseEntity<String> eliminar(@PathVariable Long id){ s.eliminarUsuario(id);
        return ResponseEntity.ok("Usuario eliminado exitosamente"); }


    @PostMapping("/clases")
    @PreAuthorize("hasAnyRole('ADMIN','ADMINISTRADOR','ENTRENADOR')")
    public ResponseEntity<Clase> crearClase(@Valid @RequestBody CrearClaseRequest r){ return ResponseEntity.ok(s.crearClase(r)); }
    @PutMapping("/clases/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','ADMINISTRADOR','ENTRENADOR')")
    public ResponseEntity<Clase> actClase(@PathVariable Long id, @Valid @RequestBody CrearClaseRequest r){ return ResponseEntity.ok(s.actualizarClase(id,r)); }
    @DeleteMapping("/clases/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','ADMINISTRADOR','ENTRENADOR')")
    public ResponseEntity<String> delClase(@PathVariable Long id){ s.eliminarClase(id); return ResponseEntity.ok("Clase eliminada exitosamente"); }
    ...
    public ResponseEntity<Ubicacion> actUb(@PathVariable Long id, @Valid @RequestBody CrearUbicacionRequest r){
        return ResponseEntity.ok(s.actualizarUbicacion(id,r)); }
    @DeleteMapping("/ubicaciones/{id}")
    public ResponseEntity<String> delUb(@PathVariable Long id){ s.eliminarUbicacion(id);
